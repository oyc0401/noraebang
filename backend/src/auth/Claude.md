...
- `/auth/mobile/challenge`: 등록된 기기에 1회용 nonce를 발급. TTL(기본 2분) 내 1회 사용 가능하며, 요청마다 nonce는 교체(Replace)된다.
...
- `DeviceChallenge` 테이블이 nonce 해시(SHA-256)를 저장해 서버 발급 nonce를 관리(2분 TTL / 1회용)한다. 기기 서명은 반드시 최신 challenge를 통과해야 하며, deviceId당 미사용 nonce는 항상 1개만 유지된다.
