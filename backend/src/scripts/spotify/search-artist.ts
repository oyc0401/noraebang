...
async function searchArtist(artistName: string) {
  ...
  try {
    ...
    results.forEach((artist, index) => {
      ...
      console.log(`    Genres: ${artist.genres.length > 0 ? artist.genres.join(', ') : 'N/A'}`);
    ...
    if (bestMatch) {
      ...
      console.log(`   Match Type: ${isExactMatch ? '✅ Exact Match' : '⚠️  Similar Match'}`);
...
if (args.length === 0) {
  ...
  console.error('');
  console.error('Usage:');
  console.error('  pnpm ts-node src/scripts/spotify/search-artist.ts <artist_name>');
  console.error('');
  console.error('Examples:');
  console.error('  pnpm ts-node src/scripts/spotify/search-artist.ts 아이유');
  console.error('  pnpm ts-node src/scripts/spotify/search-artist.ts "방탄소년단"');
  console.error('  pnpm ts-node src/scripts/spotify/search-artist.ts "영재(4MEN)"');
  process.exit(1);
}
