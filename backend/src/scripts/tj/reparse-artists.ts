...
async function main() {
  ...
  console.log(
    `Mode: ${isDryRun ? "DRY RUN (실제 업데이트 안 함)" : "LIVE (실제 업데이트)"}`,
  );
  console.log("");
  ...
  while (offset < totalCount) {
    ...
    for (const song of songs) {
      ...
      try {
        ...
        const artistChanged =
          JSON.stringify(song.artistList) !== JSON.stringify(parsed.artist);
        const featureChanged =
          JSON.stringify(song.featureList) !== JSON.stringify(parsed.feature);
        const producerChanged =
          JSON.stringify(song.producerList) !== JSON.stringify(parsed.producer);
        ...
        if (updated < 10) {
          ...
          if (artistChanged) {
            console.log(
              `     Artist: [${song.artistList.join(", ")}] → [${parsed.artist.join(", ")}]`,
            );
          }
          if (featureChanged) {
            console.log(
              `     Feat: [${song.featureList.join(", ")}] → [${parsed.feature.join(", ")}]`,
            );
          }
          if (producerChanged) {
            console.log(
              `     Prod: [${song.producerList.join(", ")}] → [${parsed.producer.join(", ")}]`,
            );
          }
        }
    ...
    // 진행 상황 출력
    console.log(
      `   진행: ${processed}/${totalCount} | 업데이트: ${updated} | 스킵: ${skipped} | 에러: ${errors}`,
    );
  }
  ...
  console.log(
    `업데이트: ${updated.toLocaleString()}곡 (${((updated / processed) * 100).toFixed(1)}%)`,
  );
