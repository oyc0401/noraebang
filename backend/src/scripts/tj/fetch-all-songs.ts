...
async function fetchAllTJSongs(fromYearMonth: string, force: boolean) {
  console.log(`🚀 Starting TJ Media ALL songs fetch (${fromYearMonth} ~ now)...\n`);
  if (force) {
  ...
  for await (const { yearMonth, songs } of tjService.fetchAllSongs(fromYearMonth)) {
    totalMonths++;
...
if (fromYearMonth !== '200101' && !/^\d{6}$/.test(fromYearMonth)) {
  console.error('❌ Error: Invalid fromYearMonth format. Expected YYYYMM (e.g., 202001)');
  console.log('Usage: pnpm ts-node src/scripts/tj/fetch-all-songs.ts [fromYearMonth] [--force]');
  process.exit(1);
}
