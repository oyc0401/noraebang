...
titleKo, titleLatin등등 넣기. (한국어/영어/일본어 구분해서 넣기) 

primary의 괄호제거 버전도 넣기. (["『ユイカ』","ユイカ"] 둘 다 primary에 넣기.)


### A(Alias) 사람이 실제로 칠 만한 신뢰도 높은 대체 입력
  - primary norm (경량 정규화: 공백 제거 / 히라↔가타 / 대소문자+공백제거+히라↔가타) (이때 중복 금지. 처리 끝나고.["Yoa", "Yoa"] 처럼 같은 배열에 중복값 제거도 하기.)

  q_song_ko_a: (공백 제거)
  q_song_ja_latin_a: (공백 제거) (대소문자는 입력전처리과정에서 모두 통일.)
  q_song_ja_kanji_a: (공백 제거 / 히라 / 가타 / 공백히라 / 공백가타) (한자지만, 안에 가타카다 히라가나 있을 수 있음.)
  q_song_ja_kana_a: (공백 제거 / 히라 / 가타 / 공백히라 / 공백가타) (히라/가타는 입력전처리때 처리 안함.)
