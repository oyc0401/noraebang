...
import {
  type TjSongCatalogFilter,
  type TjSongListQueryDto,
  type TjSongSortBy,
  type TjSongStatusFilter,
  type SortOrder,
} from "./dto/tj-song-list-query.dto";
...
@Injectable()
export class TjSongService {
  constructor(private readonly prisma: PrismaService) {}

  async findAll(query: TjSongListQueryDto): Promise<TjSongListResponseDto> {
    ...
    const rows = await this.prisma.$queryRaw<TjSongRow[]>`
      select
        t.id as tj_number,
        t.title,
        t.artist,
...
function buildOrderSql(sortBy: TjSongSortBy, sortOrder: SortOrder) {
...
function parseSortBy(value: string | undefined): TjSongSortBy {
