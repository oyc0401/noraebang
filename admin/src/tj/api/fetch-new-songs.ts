...
const TJ_NEW_SONG_OF_MONTH_URL =
  "https://www.tjmedia.com/legacy/api/newSongOfMonth";

export async function fetchTjNewSongsByYearMonth(
  yearMonth: string,
): Promise<TjSongData[]> {
  try {
    const response = await fetch(TJ_NEW_SONG_OF_MONTH_URL, {
      method: "POST",
      headers: {
        "User-Agent":
...
function isTjMonthResponse(data: unknown): data is TjMonthResponse {
  ...
  return (
    response.resultCode === "99" && Array.isArray(response.resultData?.items)
  );
}
