...
export type RealArtistSummary = Awaited<
  ReturnType<typeof getRealArtists>
>[number];
...
export async function getTjSongsByArtist(
  artistName: string,
  savedFilter: SongSavedFilter = "all",
) {
...
export async function getArtistsByExactName(
  name: string,
): Promise<NamedArtistSummary[]> {
...
export async function checkArtistConflicts(input: {
  name?: string;
  nameKo?: string;
}) {
  ...
  if (nameKo)
    conditions.push({ nameKo: { equals: nameKo, mode: "insensitive" } });
  ...
  duplicates.forEach((artist) => {
    if (name && artist.name.toLowerCase() === name.toLowerCase())
      result.name = true;
    if (nameKo && artist.nameKo.toLowerCase() === nameKo.toLowerCase())
      result.nameKo = true;
  });
