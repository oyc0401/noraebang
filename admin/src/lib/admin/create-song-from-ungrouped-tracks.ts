...
// createSongFromUngroupedTracks는 아티스트의 미그룹 SpotifyTrack들 중에서
...
export interface CreateSongFromUngroupedTracksOptions {
  dryRun?: boolean;
}

export async function createSongFromUngroupedTracks(
  artistId: number,
  options: CreateSongFromUngroupedTracksOptions = {},
): Promise<void> {
  const { dryRun = false } = options;

  console.log(
    `\n🎵 createSongFromUngroupedTracks → artistId=${artistId} ${dryRun ? "(dry-run)" : ""}`,
  );
  ...
  for (const song of existingSongs) {
    ...
    song.spotifyTrackGroup?.tracks.forEach((t) => {
  ...
  // 3. 미그룹 SpotifyTrack들 조회 (인기도 50 이상, 내림차순)
  const ungroupedTracks = await prisma.spotifyTrack.findMany({
    where: {
      disabled: false,
      groupId: null,
      popularity: { gte: 50 },
  ...
  console.log(`  • Ungrouped Tracks: ${ungroupedTracks.length}`);
  console.log(`  • Unmapped Videos: ${unmappedVideos.length}`);

  if (ungroupedTracks.length === 0 || unmappedVideos.length === 0) {
  ...
  // 6. 각 미그룹 트랙에 대해 매칭되는 비디오 찾기
  // 인기도 내림차순이므로 "A"가 먼저 처리되고, "A inst"는 정규화 중복으로 스킵됨
  type TrackVideoMatch = {
    track: UngroupedTrack;
    video: UnmappedVideo;
  };

  const trackVideoMatches: TrackVideoMatch[] = [];
  const processedVideoIds = new Set<string>();
  const processedNormalizedTitles = new Set<string>(); // 이번 루프에서 처리할 제목들
  ...
  for (const track of ungroupedTracks) {
    ...
    if (existingSong) {
      continue; // 이미 Song이 있으면 스킵
    }

    // 이번 루프에서 이미 처리 예정인 제목인지 확인 (A inst → A와 동일)
    if (processedNormalizedTitles.has(normalizedTrackTitle)) {
      continue; // 정규화된 제목이 같으면 스킵 (inst, remix 등)
    }
    ...
    if (matchedVideo && !processedVideoIds.has(matchedVideo.videoId)) {
      ...
      processedNormalizedTitles.add(normalizedTrackTitle); // 이 정규화 제목은 처리됨
    ...
    try {
      ...
      const newSong = await prisma.song.create({
        data: {
          ...
          spotifyTrackGroupId: newGroup.id,
...
async function linkRelatedTracks(
  ...
  groupId: number,
  songTitle: string,
  ungroupedTracks: UngroupedTrack[],
  excludeTrackId: number,
  dryRun: boolean,
): Promise<number> {
  const trackTitleToTrack = new Map<string, UngroupedTrack>();
  const normalizedToTrack = new Map<string, UngroupedTrack>();
