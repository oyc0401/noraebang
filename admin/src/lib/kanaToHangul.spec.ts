...
import { KanaToHangulMaker, type KanaToHangul } from "./kanaToHangul";

let kanaToHangul: KanaToHangul;

beforeAll(async () => {
  kanaToHangul = await KanaToHangulMaker.init();
});
...
describe("kanaToHangul - h row (は/ひ/ふ/へ/ほ + ひゃ/ひゅ/ひょ) stress", () => {
  ...
  it("moraic nasal ん + h-row", () => {
    ...
    expect(kanaToHangul("ひゃんひ")).toBe("햔히");
  });
...
describe("ハロ / ハロー ending words", () => {
  ...
  it("ハロー + particle は", () => {
    expect(kanaToHangul("ハローさんはあいさつです")).toBe(
      "하로상와아이사츠데스",
    );
  });
});
